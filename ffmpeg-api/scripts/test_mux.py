...
Tests local-file mode: sends one video and one audio, checks success and output filename.
Usage:
  export INTERNAL_API_KEY=your_key   # or pass --api-key
  python scripts/test_mux.py --video /source/clip.mp4 --audio /source/music.mp3 --output /source/out.mp4
  python scripts/test_mux.py --video clip.mp4 --audio music.mp3 --output out.mp4  # filenames under SOURCE_DIR
"""
import argparse
import os
import sys
...
def main():
    ...
    payload = {"video": args.video, "audio": args.audio, "output": args.output}
    ...
    if not output:
        print(f"Missing 'output' in response: {data}", file=sys.stderr)
        sys.exit(1)
